...
def test_hash_kwargs_with_unserializable_dict() -> None:
    ...
    # Create dict with unserializable object (circular reference)
...
def test_ensure_ocr_backend_registered_with_tesseract(docx_document: Path) -> None:
    ...
    # Should not raise, tesseract doesn't need registration
...
def test_ensure_ocr_backend_registered_with_easyocr_missing_dependency(docx_document: Path) -> None:
    ...
    # Mock ImportError when importing EasyOCRBackend
...
def test_ensure_ocr_backend_registered_with_paddleocr_missing_dependency(docx_document: Path) -> None:
    ...
    # Mock ImportError when importing PaddleOCRBackend
...
def test_ocr_backend_cache_eviction(docx_document: Path) -> None:
    ...
    # This test verifies the cache size limit logic (lines 186-188)
    # by registering many different backends with different kwargs
    import kreuzberg
    ...
    configs = []
    for _i in range(12):  # More than _MAX_CACHE_SIZE (10)
        config = ExtractionConfig(ocr=OcrConfig(backend="tesseract", language="eng"))
        configs.append(config)

    # Register all backends by trying extraction with different kwargs
